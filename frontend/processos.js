...
(function () {
    ...
    function processoPayload(p) {
        ...
        try {
            if (typeof processoParaSupabaseSIGEE === 'function') {
                ...
                if (inicioCiclo) {
                    payload.data_inicio_desarquivamento = inicioCiclo;
                    payload.data_inicio_ciclo = inicioCiclo;
                    payload.prazo_inicio_ciclo = inicioCiclo;
                }
    ...
    function alertaPrazo(p) {
        ...
        if (pertenceCicloDesarquivamento(etapa)) {
            if (dias >= 51) return '<span class="block bg-red-700 text-white text-[8px] font-extrabold px-1 py-0.5 rounded uppercase mt-0.5">SOLICITAR ATAS SEM PASTA</span>';
            if (dias >= 44) return '<span class="block bg-red-600 text-white text-[8px] font-extrabold px-1 py-0.5 rounded uppercase mt-0.5">CONFIRMAR DADOS DA BUSCA</span>';
            if (dias >= 37) return '<span class="block bg-orange-500 text-white text-[8px] font-extrabold px-1 py-0.5 rounded uppercase mt-0.5">REITERAÇÃO COM URGÊNCIA</span>';
            if (dias >= 30) return '<span class="block bg-amber-500 text-gray-900 text-[8px] font-extrabold px-1 py-0.5 rounded uppercase mt-0.5">REITERAÇÃO</span>';
        }
    ...
    function alertaChave(p) {
        ...
        if (dias >= 51) return 'PEDIDO_ATAS_SEM_PASTA';
        if (dias >= 44) return 'CONFIRMAR_DADOS';
        if (dias >= 37) return 'REITERACAO_URGENTE';
        if (dias >= 30) return 'REITERACAO';
...
(function(){
  ...
  function mapear(r){
    ...
    if(typeof window.processoDoSupabaseParaLocalSIGEE==='function') {
      try {
        ...
        return {
          ...
          data_inicio_desarquivamento: convertido.data_inicio_desarquivamento || r.data_inicio_desarquivamento || null,
          data_inicio_ciclo: convertido.data_inicio_ciclo || r.data_inicio_ciclo || null,
          prazo_inicio_ciclo: convertido.prazo_inicio_ciclo || r.prazo_inicio_ciclo || null,
          inicio_ciclo: convertido.inicio_ciclo || r.inicio_ciclo || null
        };
    ...
    return {
      ...
      data_inicio_desarquivamento: r.data_inicio_desarquivamento || null,
      data_inicio_ciclo: r.data_inicio_ciclo || null,
      prazo_inicio_ciclo: r.prazo_inicio_ciclo || null,
      inicio_ciclo: r.inicio_ciclo || null,
