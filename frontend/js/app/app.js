  ...
  async function carregarBaseUsuarioCore(){
    ...
    try{
      const [escolas, processos, usuarios] = await Promise.all([
        ...
        (async()=>{
          if(window.SIGEE_RELATORIOS_DADOS?.buscarTodos){
            const nteAlvo = isGlobal(u) ? null : nteIdUsuario(u);
            const dados = await window.SIGEE_RELATORIOS_DADOS.buscarTodos({nte:nteAlvo,forcar:true});
            return (dados||[]).map(normalizarProcesso);
          }
          const todos=[]; const pagina=1000; let inicio=0;
          while(true){
            let q = c.from(T.processos).select('*').order('created_at', { ascending: false }).range(inicio,inicio+pagina-1);
            if(!isGlobal(u)) q = q.eq('nte', nteTexto(nteIdUsuario(u)));
            const {data,error} = await q; if(error) throw error;
            todos.push(...(data||[])); if(!data||data.length<pagina) break; inicio+=pagina;
          }
          return todos.map(normalizarProcesso);
        })(),
        (async()=>{
