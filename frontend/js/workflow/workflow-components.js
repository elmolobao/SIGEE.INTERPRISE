/* =====================================================================
   SIGEE Sprint 3.7.3 — Template Oficial das Janelas
   Mantém eventos e campos; apenas normaliza a ordem visual.
   ===================================================================== */
(function(){
  'use strict';
  if(window.__SIGEE_WORKFLOW_COMPONENTS_373__)return;
  window.__SIGEE_WORKFLOW_COMPONENTS_373__=true;
  ...
  function padronizar(modal){
    ...
    /*
     * Correção 3.7.3: a reorganização é idempotente.
     * O MutationObserver é acionado pelas próprias movimentações do DOM;
     * sem esta trava, uma segunda execução removia os contêineres já montados
     * e fazia o conteúdo desaparecer após um breve instante.
     */
    const estruturaPronta =
      b.querySelector(':scope > .sigee-janela-conteudo') ||
      b.querySelector(':scope > .sigee-janela-rodape');
    if(estruturaPronta){
      modal.dataset.sigeeTemplate373='1';
      return;
    }

    /* Reaplica somente quando o conteúdo original do modal for reconstruído. */
  ...
  console.info('[SIGEE] Template oficial de janelas 3.7.3 carregado.');
})();
