...
(function () {
  ...
  // RC12.0.3C — padronização visual do acervo no cadastro de instituições.
  // A situação funcional é a autoridade para escolas ativas; valores legados
  // "ACOLHIDO"/"NÃO ACOLHIDO" são apenas normalizados na apresentação,
  // sem qualquer escrita ou migração no banco.
  function acervoVisual(e) {
    const situacao = normalizar(escolaSituacao(e));
    const acervo = normalizar(escolaAcervo(e));

    if (situacao.includes('ATIVA')) {
      return { rotulo: 'ESCOLA ATIVA', tipo: 'ativa', local: 'ESCOLA ATIVA' };
    }

    const negativo = acervo.includes('NAO') || acervo.includes('NÃO');
    const recolhido = acervo.includes('RECOLHIDO') || acervo.includes('ACOLHIDO');

    if (recolhido && !negativo) {
      return { rotulo: 'RECOLHIDO', tipo: 'recolhido', local: escolaLocal(e) || 'ACERVO DO NTE' };
    }

    if ((recolhido && negativo) || acervo.includes('A RECOLHER')) {
      return { rotulo: 'NÃO RECOLHIDO', tipo: 'nao-recolhido', local: escolaLocal(e) };
    }

    return { rotulo: escolaAcervo(e) || '-', tipo: 'neutro', local: escolaLocal(e) };
  }
  ...
  function badge(valor, tipo) {
    ...
    if (tipo === 'acervo-ativa') {
      cls = 'text-white';
      style = 'background:#475569;color:#fff;';
    }
    if (tipo === 'acervo-recolhido') {
      // Azul mais luminoso + texto branco para garantir contraste.
      cls = 'text-white';
      style = 'background:#2563eb;color:#fff;box-shadow:0 2px 8px rgba(37,99,235,.22);';
    }
    if (tipo === 'acervo-nao-recolhido') {
      cls = 'bg-red-600 text-white';
    }
    if (tipo === 'acervo') {
      if (n.includes('RECOLHIDO') || n.includes('ACOLHIDO')) cls = 'bg-blue-600 text-white';
      if (n.includes('NAO') || n.includes('NÃO')) cls = 'bg-red-600 text-white';
    }
    return `<span class="px-2 py-1 rounded-full text-[10px] font-black ${cls}"${style ? ` style="${style}"` : ''}>${escapeHtml(v.toUpperCase())}</span>`;
  }
  ...
  function desenharTabela(lista) {
    ...
    corpo.innerHTML = (lista || []).map(e => {
      ...
      return `<tr class="hover:bg-white/10 text-[11px] text-white border-b border-white/10 ${inativa ? 'opacity-60' : ''}">
        ...
        <td class="p-3">${(() => { const av = acervoVisual(e); return `${badge(av.rotulo, `acervo-${av.tipo}`)}<br><small class="text-slate-600">${escapeHtml(av.local)}</small>`; })()}</td>
...
Esta ação não pode ser desfeita.`)) return;
   ...
   * A pesquisa, seleção e limpeza são controladas exclusivamente pelo
   * módulo reorganizado instalado ao final deste arquivo.
   */
...
(function instalarNovaSolicitacaoDigitavelSIGEE() {
    ...
    function obterEscolaSelecionadaOficial() {
        return escolaSelecionadaOficial ? { ...escolaSelecionadaOficial } : null;
    }

    function definirEscolaSelecionadaOficial(escola) {
        escolaSelecionadaOficial = escola ? { ...escola } : null;
        return obterEscolaSelecionadaOficial();
    }
    ...
    function preencherEscolaNovaSolicitacao(escola) {
        ...
        if (idOculto) {
            idOculto.value = escolaId;
            idOculto.dataset.abertura = String(window.SIGEE_NOVA_SOLICITACAO_ABERTURA_ATUAL || 0);
        }
        if (input) input.dataset.abertura = String(window.SIGEE_NOVA_SOLICITACAO_ABERTURA_ATUAL || 0);
        if (select) select.dataset.abertura = String(window.SIGEE_NOVA_SOLICITACAO_ABERTURA_ATUAL || 0);
        definirEscolaSelecionadaOficial({
            ...escola,
            id: escolaId,
            escola_id: escolaId,
            nome: escola.nome,
        ...
        window.SIGEE_ESCOLA_NOVA_SOLICITACAO = {
            ...
            __abertura: Number(window.SIGEE_NOVA_SOLICITACAO_ABERTURA_ATUAL || 0)
        };
    ...
    function prepararCampoEscolaNovaSolicitacao() {
        ...
        input.oninput = function () {
            ...
            if (idOculto) {
                idOculto.value = '';
                delete idOculto.dataset.abertura;
            }
            delete input.dataset.escolaId;
            delete input.dataset.codMec;
            delete input.dataset.abertura;
            delete select.dataset.abertura;
            definirEscolaSelecionadaOficial(null);
    ...
    function limparEstadoNovaSolicitacaoDefinitivo() {
        const input = campo('novo-proc-escola-busca-v23');
        const select = campo('novo-proc-escola');
        const lista = campo('novo-proc-escola-lista-v23');
        const idOculto = campo('novo-proc-escola-id');
    ...
    function abrirNovaSolicitacaoOficial() {
        ...
        window.SIGEE_NOVA_SOLICITACAO_ABERTURA_ATUAL =
            Number(window.SIGEE_NOVA_SOLICITACAO_ABERTURA_ATUAL || 0) + 1;

        prepararCampoEscolaNovaSolicitacao();
        protegerCamposPesquisaEscolaContraAutofill();
        limparEstadoNovaSolicitacaoDefinitivo();
    ...
    function instalarAutoridadeOficialNovaSolicitacao() {
        ...
        window.handleSelecaoInstituicaoFluxoAutomatico = function () {
            const estado = obterEscolaSelecionadaOficial();
            const idOculto = campo('novo-proc-escola-id');
            if (!estado || !texto(estado.id || estado.escola_id)) {
                if (idOculto) idOculto.value = '';
                return false;
            }
            if (idOculto) idOculto.value = texto(estado.id || estado.escola_id);
            return true;
        };

        window.SIGEE_Escolas = window.SIGEE_Escolas || {};
        window.SIGEE_Escolas.obterEscolaNovaSolicitacao = obterEscolaSelecionadaOficial;
        window.SIGEE_Escolas.limparEscolaNovaSolicitacao = limparEstadoNovaSolicitacaoDefinitivo;
    }
