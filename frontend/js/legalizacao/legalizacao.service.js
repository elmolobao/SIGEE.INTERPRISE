...
async function iniciarProcedimentoOferta(instituicaoId,payload={}){
  ...
  const competencia=competenciaProcedimentoOferta(catalogo||[]);const tipoProcesso=['RENOVACAO','RENOVACAO_RECONHECIMENTO'].includes(subtipo)?'RENOVACAO':'AUTORIZACAO';const sei=clean(payload.numero_sei),dataProtocolo=clean(payload.data_protocolo);if(!sei)throw new Error('Informe o Processo SEI do ato regulatório.');if(!dataProtocolo)throw new Error('Informe a data de protocolo do Processo SEI.');const base={instituicao_id:inst.id,nte_id:inst.nte_id,tipo:tipoProcesso,subtipo,numero_sei:sei,competencia_regulatoria:competencia.competencia_regulatoria,orgao_instrutor:competencia.orgao_instrutor,status:'EM_ANDAMENTO',etapa_atual:'INSTRUCAO_DOCUMENTAL',data_protocolo:dataProtocolo,prazo_etapa:clean(payload.prazo_etapa),observacao:clean(payload.observacao),responsavel_id:currentUserId(),criado_por_id:currentUserId(),atualizado_por_id:currentUserId()};
...
async function iniciarCredenciamento(instituicaoId,payload={}){
  ...
  const sei=clean(payload.numero_sei),dataProtocolo=clean(payload.data_protocolo);if(!sei)throw new Error('Informe o Processo SEI do ato regulatório.');if(!dataProtocolo)throw new Error('Informe a data de protocolo do Processo SEI.');
  ...
  const base={instituicao_id:inst.id,nte_id:inst.nte_id,tipo:'CREDENCIAMENTO',numero_sei:sei,status:'EM_ANDAMENTO',etapa_atual:'RECEBIMENTO_DOCUMENTAL',data_protocolo:dataProtocolo,observacao:obsOriginal,criado_por_id:currentUserId(),atualizado_por_id:currentUserId()};
  ...
  await historicoProcesso(p.id,'ABERTURA',`${subtipo==='RECREDENCIAMENTO'?'Recredenciamento':'Credenciamento'} iniciado no SIGEE para recebimento documental.`,{numero_sei:sei,data_protocolo:dataProtocolo,tipo_cadastro:inst.tipo_cadastro,subtipo,prazo_etapa:clean(payload.prazo_etapa),situacao_imovel:situacaoImovel,mantenedora:mantenedora.razao_social});resumoCache=null;return p;
}
...
async function iniciarAlteracaoCadastral(instituicaoId,payload={}){assertAccess();const inst=await oneScoped('legalizacao_instituicoes',instituicaoId),c=client();const subt=String(payload.subtipo||'OUTRA').toUpperCase();if(!['ENDERECO','DENOMINACAO','MANTENEDORA','RESPONSAVEIS','OUTRA'].includes(subt))throw new Error('Tipo de alteração cadastral inválido.');if(!clean(payload.observacao))throw new Error('Descreva a alteração cadastral.');const sei=clean(payload.numero_sei),dataProtocolo=clean(payload.data_protocolo);if(!sei)throw new Error('Informe o Processo SEI do ato regulatório.');if(!dataProtocolo)throw new Error('Informe a data de protocolo do Processo SEI.');const now=new Date().toISOString(),row={instituicao_id:inst.id,nte_id:inst.nte_id,tipo:'ALTERACAO_CADASTRAL',subtipo:subt,numero_sei:sei,data_protocolo:dataProtocolo,prazo_etapa:clean(payload.prazo_etapa),observacao:clean(payload.observacao),status:'EM_ANDAMENTO',etapa_atual:'ANALISE_CADASTRAL',responsavel_id:currentUserId(),criado_por_id:currentUserId(),atualizado_por_id:currentUserId(),created_at:now,updated_at:now};const {data,error}=await c.from('legalizacao_processos').insert(row).select('*').single();if(error)throw error;await historicoProcesso(data.id,'ABERTURA_ALTERACAO_CADASTRAL','Procedimento de alteração cadastral aberto com Processo SEI vinculado.',{subtipo:subt,numero_sei:sei,data_protocolo:dataProtocolo});return data;}
...
async function localizarInstituicoesParaVinculoAto(filtros={}){
  ...
  // Fallback no cadastro mestre para MEC/INEP ou nome quando o prontuário ainda não foi habilitado.
  if(!out.length&&(tipo==='INEP'||tipo==='NOME')){
    const c=client();let q=c.from('escolas_sigee').select('id,nome_escola,nome,municipio,nte_id,cod_mec').limit(50);
    if(tipo==='INEP')q=q.eq('cod_mec',alvo);else{const nomeBusca=String(valor).trim().replace(/[,()]/g,' ');q=q.or(`nome_escola.ilike.%${nomeBusca}%,nome.ilike.%${nomeBusca}%`);}
...
async function aplicarEfeitoRegulatorioAtoConfirmado(inst,r,escolaLegada){
  const c=client(),tipo=normalizarOfertaAto([r.tipo_ato,r.ato].filter(Boolean).join(' ')),now=new Date().toISOString();
  ...
  if(situacao){const {error}=await c.from('legalizacao_instituicoes').update({situacao_regulatoria:situacao,atualizado_por_id:currentUserId(),updated_at:now}).eq('id',inst.id);if(error)throw error;
    if(situacao==='EXTINTA'&&escolaLegada){try{await c.from('escolas_sigee').update({situacao_funcional:'Extinta'}).eq('id',escolaLegada);}catch(e){console.warn('[DOE] não foi possível refletir situação funcional no catálogo legado',e);}}
  }
  const ini=anoIso(r.vigencia_inicio),fim=anoIso(r.vigencia_fim);if(!ini&&!fim)return;
  const alvos=[];if(tipo.includes('FUNDAMENTAL I')||tipo.includes('ANOS INICIAIS'))alvos.push('FUNDAMENTAL I','ANOS INICIAIS');if(tipo.includes('FUNDAMENTAL II')||tipo.includes('ANOS FINAIS'))alvos.push('FUNDAMENTAL II','ANOS FINAIS');if(tipo.includes('ENSINO MEDIO'))alvos.push('ENSINO MEDIO');if(tipo.includes('EDUCACAO INFANTIL'))alvos.push('EDUCACAO INFANTIL');if(tipo.includes('TECNIC'))alvos.push('TECNIC');if(!alvos.length)return;
  const {data:ofs,error:eo}=await c.from('legalizacao_ofertas').select('id,etapa_modalidade,curso_tecnico').eq('instituicao_id',inst.id);if(eo){console.warn('[DOE] ofertas não puderam ser consultadas',eo);return;}
  const ids=(ofs||[]).filter(o=>{const t=normalizarOfertaAto([o.etapa_modalidade,o.curso_tecnico].filter(Boolean).join(' '));return alvos.some(a=>t.includes(a));}).map(o=>o.id);if(!ids.length)return;
  const upd={situacao:'AUTORIZADA',updated_at:now};if(ini)upd.ano_inicio_vigencia=ini;if(fim)upd.ano_fim_vigencia=fim;const {error:eu}=await c.from('legalizacao_ofertas').update(upd).in('id',ids);if(eu)throw eu;
  ...
  if(clean(r.numero_processo)){const {data:procs,error:ep}=await c.from('legalizacao_processos').select('id,tipo,subtipo,status,etapa_atual,numero_sei,instituicao_id').eq('instituicao_id',inst.id).eq('numero_sei',clean(r.numero_processo)).order('created_at',{ascending:false}).limit(10);if(ep)throw ep;const candidatos=(procs||[]).filter(x=>!['CONCLUIDO','CANCELADO'].includes(upper(x.status)));if(candidatos.length===1){const p0=candidatos[0],upd={tipo_ato:clean(r.tipo_ato)||clean(r.ato)||p0.subtipo||p0.tipo,numero_ato:clean(r.numero_publicacao),data_publicacao:clean(r.data_publicacao),referencia_doe:`Importação DOE: ${clean(r.arquivo_origem)}`,vigencia_inicio:clean(r.vigencia_inicio),vigencia_fim:clean(r.vigencia_fim),publicado_em:now,publicado_por_id:currentUserId(),status:'CONCLUIDO',etapa_atual:'ATO_PUBLICADO',atualizado_por_id:currentUserId(),updated_at:now};if(upper(p0.tipo)==='CREDENCIAMENTO'){upd.etapa_atual='CREDENCIAMENTO_CONCLUIDO';upd.credenciamento_concluido_em=now;upd.credenciamento_concluido_por_id=currentUserId();}const {error:eup}=await c.from('legalizacao_processos').update(upd).eq('id',p0.id);if(eup)throw eup;await historicoProcesso(p0.id,'PUBLICACAO_DOE_IMPORTADA','Publicação do Diário Oficial importada, conferida e vinculada ao Processo SEI.',{numero_ato:r.numero_publicacao,data_publicacao:r.data_publicacao,importacao_id:r.id});if(upper(p0.tipo)==='CREDENCIAMENTO'){const {error:ei}=await c.from('legalizacao_instituicoes').update({situacao_regulatoria:'CREDENCIADA',atualizado_por_id:currentUserId(),updated_at:now}).eq('id',inst.id);if(ei)throw ei;}}else if(candidatos.length>1){throw new Error('Há mais de um procedimento ativo com este Processo SEI. A publicação foi importada, mas a oficialização exige vinculação inequívoca.');}}atosControleCache=null;resumoCache=null;return data;}
