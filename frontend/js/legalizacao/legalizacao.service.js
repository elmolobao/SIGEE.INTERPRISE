...
async function iniciarCredenciamento(instituicaoId,payload={}){
  ...
  await historicoProcesso(p.id,'ABERTURA',`${subtipo==='RECREDENCIAMENTO'?'Recredenciamento':'Credenciamento'} iniciado no SIGEE para recebimento documental.`,{numero_sei:null,data_protocolo:null,tipo_cadastro:inst.tipo_cadastro,subtipo,prazo_etapa:clean(payload.prazo_etapa),situacao_imovel:situacaoImovel,mantenedora:mantenedora.razao_social,fluxo:'CHECKLIST_SEI_INSPECAO_DOE'});resumoCache=null;return p;
}
...
async function registrarProcessoSeiOferta(processoId,payload={}){
  ...
  const {data:it,error:ei}=await c.from('legalizacao_oferta_requisitos_processo').select('status,requisito_catalogo_id').eq('processo_id',p.id);if(ei)throw ei;const ids=[...new Set((it||[]).map(x=>x.requisito_catalogo_id).filter(Boolean))];let obrig=new Set();if(ids.length){const {data:cats,error:ec}=await c.from('legalizacao_oferta_requisitos_catalogo').select('id,obrigatorio').in('id',ids);if(ec)throw ec;obrig=new Set((cats||[]).filter(x=>x.obrigatorio).map(x=>String(x.id)));}
  const pend=(it||[]).filter(x=>obrig.has(String(x.requisito_catalogo_id))&&!['APRESENTADO','CONFORME','NAO_SE_APLICA'].includes(upper(x.status)));if(pend.length)throw new Error(`Ainda existem ${pend.length} requisito(s) obrigatório(s) não encaminhado(s). O Processo SEI só pode ser registrado após a juntada documental.`);
