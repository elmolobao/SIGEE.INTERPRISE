...
async function listarDescredenciamentosRegulatorios(){
  ...
  const procIds=base.map(x=>Number(x.id)).filter(Boolean);let handoffMap=new Map(),acervoMap=new Map();if(procIds.length){const hr=await c.from('extintas_descredenciamentos').select('id,legalizacao_processo_id,status,etapa_atual,escola_id,updated_at').in('legalizacao_processo_id',procIds).neq('status','CANCELADA_ERRO');if(!hr.error){handoffMap=new Map((hr.data||[]).map(h=>[String(h.legalizacao_processo_id),h]));const chamadoIds=(hr.data||[]).map(h=>Number(h.id)).filter(Boolean);if(chamadoIds.length){const ar=await c.from('extintas_acervo_recolhimentos').select('chamado_id,situacao,fase_atual,data_recolhimento,recolhimento_agendado_em,visita_preventiva_em,orientacao_em,local_guarda,unidade_recebedora,updated_at').in('chamado_id',chamadoIds);if(!ar.error)acervoMap=new Map((ar.data||[]).map(a=>[String(a.chamado_id),a]));}}}
  return await Promise.all(base.map(async x=>{const handoff=handoffMap.get(String(x.id))||null,acervo=handoff?acervoMap.get(String(handoff.id))||null:null;const ext={extintas_chamado_id:handoff?.id||null,extintas_status:handoff?.status||null,extintas_etapa:handoff?.etapa_atual||null,extintas_acervo_situacao:acervo?.situacao||null,extintas_acervo_fase:acervo?.fase_atual||null,extintas_acervo_data_recolhimento:acervo?.data_recolhimento||null,extintas_recolhimento_agendado_em:acervo?.recolhimento_agendado_em||null,extintas_visita_preventiva_em:acervo?.visita_preventiva_em||null,extintas_orientacao_em:acervo?.orientacao_em||null,extintas_acervo_local:acervo?.local_guarda||null,extintas_acervo_unidade:acervo?.unidade_recebedora||null};try{return {...x,...ext,checklist:await garantirChecklistDescredenciamento(x)};}catch(err){console.warn('[Legalização] checklist de descredenciamento indisponível',x.id,err);return {...x,...ext,checklist:[],checklist_erro:String(err?.message||err||'Falha ao carregar checklist')};}}));
}

async function obterHistoricoDescredenciamentoIntegrado(processoId){
  assertAccess();const c=client();let q=c.from('legalizacao_processos').select('*').eq('id',Number(processoId)).eq('tipo','DESCREDENCIAMENTO');q=scoped(q);const {data:p,error:ep}=await q.maybeSingle();if(ep)throw ep;if(!p)throw new Error('Descredenciamento não localizado na sua abrangência.');
  const eventos=[];
  const lh=await c.from('legalizacao_processos_historico').select('*').eq('processo_id',Number(processoId)).order('created_at',{ascending:true});
  if(!lh.error)for(const h of (lh.data||[]))eventos.push({origem:'LEGALIZACAO',data:h.created_at||null,tipo:h.tipo||'REGISTRO',descricao:h.descricao||'',usuario_id:h.usuario_id||null,meta:h.meta||null});
  const eh=await c.from('extintas_descredenciamentos').select('*').eq('legalizacao_processo_id',Number(processoId)).neq('status','CANCELADA_ERRO').order('created_at',{ascending:true});
  if(!eh.error)for(const ch of (eh.data||[])){
    const hh=await c.from('extintas_descredenciamento_historico').select('*').eq('chamado_id',Number(ch.id)).order('created_at',{ascending:true});
    if(!hh.error)for(const h of (hh.data||[]))eventos.push({origem:'ESCOLAS_EXTINTAS',data:h.created_at||null,tipo:h.evento||'REGISTRO',descricao:h.descricao||'',usuario_nome:h.usuario_nome||null,usuario_id:h.usuario_id||null});
    const ar=await c.from('extintas_acervo_recolhimentos').select('*').eq('chamado_id',Number(ch.id)).order('created_at',{ascending:false}).limit(1);
    if(!ar.error&&ar.data?.[0]){const a=ar.data[0];if(a.orientacao_em)eventos.push({origem:'ESCOLAS_EXTINTAS',data:a.orientacao_em,tipo:'ORIENTACAO',descricao:'Orientação para tratamento/recolhimento do acervo registrada.'});if(a.visita_preventiva_em)eventos.push({origem:'ESCOLAS_EXTINTAS',data:a.visita_preventiva_em,tipo:'VISITA_PREVENTIVA',descricao:'Visita/inspeção preventiva programada para tratamento do acervo.'});if(a.recolhimento_agendado_em)eventos.push({origem:'ESCOLAS_EXTINTAS',data:a.recolhimento_agendado_em,tipo:'RECOLHIMENTO_AGENDADO',descricao:'Recolhimento do acervo agendado.'});if(a.data_recolhimento)eventos.push({origem:'ESCOLAS_EXTINTAS',data:a.data_recolhimento,tipo:'RECOLHIDO',descricao:'Recolhimento do acervo registrado como realizado.'});}
  }
  eventos.sort((a,b)=>String(a.data||'').localeCompare(String(b.data||'')));return {processo:p,eventos};
}
...
async function atualizarOfertaCredenciamento(ofertaId,payload={}){
  assertAccess();if(!podeGerirDoe())throw new Error('A correção de curso vinculado é exclusiva dos perfis SEC e MASTER.');
  const c=client(),{data:oferta,error:eo}=await c.from('legalizacao_processos_ofertas').select('*').eq('id',ofertaId).maybeSingle();if(eo)throw eo;if(!oferta)throw new Error('Curso vinculado não localizado.');
  const {data:p,error:ep}=await c.from('legalizacao_processos').select('id,tipo,status,numero_sei').eq('id',oferta.processo_id).maybeSingle();if(ep)throw ep;if(!p||upper(p.tipo)!=='CREDENCIAMENTO')throw new Error('O vínculo não pertence a um credenciamento.');if(['CONCLUIDO','ARQUIVADO','CANCELADO'].includes(upper(p.status)))throw new Error('O credenciamento já está encerrado.');
  const forma=upper(payload.forma_articulacao);if(!['INTEGRADA','CONCOMITANTE','SUBSEQUENTE'].includes(forma))throw new Error('Forma de articulação inválida.');
  const inst=await oneScoped('legalizacao_instituicoes',oferta.instituicao_id,{globalDoe:true});if(forma==='INTEGRADA'&&!redeEstadual(inst))throw new Error('A forma integrada está disponível neste fluxo somente para instituições da Rede Estadual.');
  const anterior={...(oferta.detalhamento_oferta||{})},novo={...anterior,forma_articulacao:forma};
  const {data,error}=await c.from('legalizacao_processos_ofertas').update({detalhamento_oferta:novo,updated_at:new Date().toISOString()}).eq('id',oferta.id).select('*').single();if(error)throw error;
  await historicoProcesso(p.id,'OFERTA_CORRIGIDA','Forma de articulação de curso técnico corrigida por SEC/MASTER.',{oferta_id:oferta.id,curso:oferta.curso_nome,forma_anterior:anterior.forma_articulacao||null,forma_nova:forma,usuario_id:currentUserId()});return data;
}
async function excluirOfertaCredenciamento(ofertaId){
  assertAccess();if(!podeGerirDoe())throw new Error('A exclusão de curso vinculado é exclusiva dos perfis SEC e MASTER.');
  const c=client(),{data:oferta,error:eo}=await c.from('legalizacao_processos_ofertas').select('*').eq('id',ofertaId).maybeSingle();if(eo)throw eo;if(!oferta)throw new Error('Curso vinculado não localizado.');
  const {data:p,error:ep}=await c.from('legalizacao_processos').select('id,tipo,status').eq('id',oferta.processo_id).maybeSingle();if(ep)throw ep;if(!p||upper(p.tipo)!=='CREDENCIAMENTO')throw new Error('O vínculo não pertence a um credenciamento.');if(['CONCLUIDO','ARQUIVADO','CANCELADO'].includes(upper(p.status)))throw new Error('O credenciamento já está encerrado.');
  const snapshot={curso:oferta.curso_nome,eixo:oferta.eixo_tecnologico,detalhamento:oferta.detalhamento_oferta,oferta_catalogo_id:oferta.oferta_catalogo_id};
  const {error:ed}=await c.from('legalizacao_processos_ofertas').delete().eq('id',oferta.id);if(ed)throw ed;
  const {data:restantes,error:er}=await c.from('legalizacao_processos_ofertas').select('oferta_catalogo_id').eq('processo_id',p.id);if(er)throw er;const ids=[...new Set((restantes||[]).map(x=>Number(x.oferta_catalogo_id)).filter(Boolean))];let aplicaveis=[];
  if(ids.length){const {data:cats,error:ec}=await c.from('legalizacao_oferta_catalogo').select('*').in('id',ids);if(ec)throw ec;const {data:reqs,error:eq}=await c.from('legalizacao_oferta_requisitos_catalogo').select('*').eq('ativo',true).or(`oferta_catalogo_id.is.null,oferta_catalogo_id.in.(${ids.join(',')})`);if(eq)throw eq;aplicaveis=(reqs||[]).filter(r=>requisitoOfertaAplicavel(r,cats||[],'AUTORIZACAO')).map(r=>Number(r.id));}
  const {data:atuais,error:ea}=await c.from('legalizacao_oferta_requisitos_processo').select('id,requisito_catalogo_id').eq('processo_id',p.id);if(ea)throw ea;const remover=(atuais||[]).filter(x=>!aplicaveis.includes(Number(x.requisito_catalogo_id))).map(x=>x.id);if(remover.length){const {error:ex}=await c.from('legalizacao_oferta_requisitos_processo').delete().in('id',remover);if(ex)throw ex;}
  await historicoProcesso(p.id,'OFERTA_EXCLUIDA','Curso técnico vinculado excluído por SEC/MASTER para correção de lançamento.',{oferta_id:oferta.id,...snapshot,usuario_id:currentUserId()});return true;
}
...
async function criarInstituicao(payload){
  ...
  const tipo=anexo?'PUBLICA':upper(payload.tipo_cadastro||'PUBLICA'),rede=tipo==='PRIVADA'?'PRIVADA':upper(payload.rede||'ESTADUAL'),regime=upper(payload.regime_educacional||'EDUCACAO_BASICA');
  if(!['EDUCACAO_BASICA','EDUCACAO_PROFISSIONAL_TECNICA'].includes(regime))throw new Error('Selecione Ensino regular ou Ensino técnico.');
  ...
  const registro={nte_id:Number(n),nome_instituicao:anexo?String(sede.nome_escola||sede.nome||'').trim():String(payload.nome_instituicao||'').trim(),tipo_cadastro:tipo,rede,regime_educacional:regime,tipo_unidade:anexo?'ANEXO':'SEDE',escola_sede_id:anexo?Number(sede.id):null,
...
async function cancelarProcedimentoRegulatorio(processoId,motivo){
  assertAccess();
  if(!(master()||sec()))throw new Error('O cancelamento de procedimento é exclusivo dos perfis MASTER e SEC.');
  const justificativa=clean(motivo);if(!justificativa)throw new Error('Informe o motivo do cancelamento.');
  const c=client();
  const {data:p,error:ep}=await c.from('legalizacao_processos').select('*').eq('id',processoId).maybeSingle();if(ep)throw ep;if(!p)throw new Error('Procedimento regulatório não localizado.');
  if(['CANCELADO','CONCLUIDO','PUBLICADO','ARQUIVADO'].includes(upper(p.status))||['PUBLICADO','ATO_PUBLICADO','CREDENCIAMENTO_CONCLUIDO'].includes(upper(p.etapa_atual)))throw new Error('Este procedimento já está encerrado e não pode ser cancelado por esta operação.');
  const {count:atos,error:eAt}=await c.from('legalizacao_atos_legais').select('id',{count:'exact',head:true}).eq('instituicao_id',p.instituicao_id);if(eAt&&!/relation|column|schema cache|does not exist/i.test(String(eAt.message||'')))throw eAt;
  if((atos||0)>0&&p.numero_sei){const ar=await c.from('legalizacao_atos_legais').select('id').eq('instituicao_id',p.instituicao_id).eq('numero_processo',p.numero_sei).limit(1);if(!ar.error&&ar.data?.length)throw new Error('Cancelamento não permitido. Existe ato publicado vinculado a este Processo SEI.');}
  const now=new Date().toISOString();
  const ir=await c.from('legalizacao_inspecoes').select('id,status,data_agendada,data_realizada').eq('processo_id',p.id);if(ir.error&&!/relation|column|schema cache|does not exist/i.test(String(ir.error.message||'')))throw ir.error;
  const agendamentos=(ir.data||[]).filter(x=>x.data_agendada&&!x.data_realizada&&!['CONCLUIDA','CANCELADA'].includes(upper(x.status))).map(x=>({id:x.id,data_agendada:x.data_agendada,status:x.status}));
  if((ir.data||[]).length){const ur=await c.from('legalizacao_inspecoes').update({status:'CANCELADA',data_agendada:null,updated_at:now}).eq('processo_id',p.id).is('data_realizada',null);if(ur.error&&!/relation|column|schema cache|does not exist/i.test(String(ur.error.message||'')))throw ur.error;}
  const hr=await c.from('extintas_descredenciamentos').select('id,status,etapa_atual').eq('legalizacao_processo_id',p.id);if(!hr.error&&hr.data?.length){for(const h of hr.data){if(!['CONCLUIDO','ATO_PUBLICADO','CANCELADA_ERRO'].includes(upper(h.status))){const u=await c.from('extintas_descredenciamentos').update({status:'CANCELADA_ERRO',etapa_atual:'CANCELADO',updated_at:now}).eq('id',h.id);if(u.error&&!/relation|column|schema cache|does not exist/i.test(String(u.error.message||'')))throw u.error;}}}
  const obs=[clean(p.observacao),`CANCELADO: ${justificativa}`].filter(Boolean).join('\n');
  const {data,error}=await c.from('legalizacao_processos').update({status:'CANCELADO',etapa_atual:'CANCELADO',observacao:obs,atualizado_por_id:currentUserId(),updated_at:now}).eq('id',p.id).select('*').single();if(error)throw error;
  await historicoProcesso(p.id,'PROCEDIMENTO_CANCELADO','Procedimento regulatório cancelado por MASTER/SEC. Agendamentos futuros vinculados foram retirados da agenda.',{motivo:justificativa,status_anterior:p.status,etapa_anterior:p.etapa_atual,agendamentos_cancelados:agendamentos});
  try{const u=user();await c.from('logs_sigee').insert({usuario_id:currentUserId(),nome:u?.nome||null,email:u?.email||null,acao:'Procedimento regulatório cancelado.',created_at:now,nte:String(p.nte_id||''),perfil:u?.perfil||null,detalhes:`Processo ${p.id}${p.numero_sei?' · SEI '+p.numero_sei:''} · Motivo: ${justificativa} · Agendamentos retirados: ${agendamentos.length}`,modulo:'legalizacao',etapa:'CANCELAMENTO',sessao_id:window.SIGEE_SESSAO_ID||null});}catch(e){console.warn('[Legalização] cancelamento concluído, mas log complementar falhou:',e);}
  resumoCache=null;return {...data,agendamentos_cancelados:agendamentos.length};
}
async function excluirInstituicao(instituicaoId){
  ...
  const {data:processos,error:eProc}=await c.from('legalizacao_processos').select('id,status').eq('instituicao_id',inst.id);
  if(eProc&&!/relation|column|schema cache|does not exist/i.test(String(eProc.message||'')))throw eProc;
  const pids=(processos||[]).map(x=>x.id).filter(Boolean),naoCancelados=(processos||[]).filter(x=>upper(x.status)!=='CANCELADO');
  if(naoCancelados.length)throw new Error('Exclusão não permitida. Cancele primeiro todos os procedimentos regulatórios vinculados ao cadastro.');
  if(pids.length){
    const irp=await c.from('legalizacao_inspecoes').select('id').in('processo_id',pids);if(irp.error&&!/relation|column|schema cache|does not exist/i.test(String(irp.error.message||'')))throw irp.error;const inspIds=(irp.data||[]).map(x=>x.id).filter(Boolean);
    if(inspIds.length){const r=await c.from('legalizacao_inspecao_itens').delete().in('inspecao_id',inspIds);if(r.error&&!/relation|column|schema cache|does not exist/i.test(String(r.error.message||'')))throw r.error;}
    for(const [tabela,coluna] of [['legalizacao_checklist_historico','processo_id'],['legalizacao_checklist_processo','processo_id'],['legalizacao_oferta_requisitos_processo','processo_id'],['legalizacao_processos_ofertas','processo_id'],['legalizacao_descredenciamento_ofertas','processo_id'],['legalizacao_handoff_acervo','processo_id'],['legalizacao_processos_historico','processo_id']]){const r=await c.from(tabela).delete().in(coluna,pids);if(r.error&&!/relation|column|schema cache|does not exist/i.test(String(r.error.message||'')))throw r.error;}
    const ri=await c.from('legalizacao_inspecoes').delete().in('processo_id',pids);if(ri.error&&!/relation|column|schema cache|does not exist/i.test(String(ri.error.message||'')))throw ri.error;
    const rp=await c.from('legalizacao_processos').delete().in('id',pids);if(rp.error)throw rp.error;
  }
...
async function iniciarCredenciamento(instituicaoId,payload={}){
  assertOperacaoNte();
  assertAccess();const c=client(),inst=await oneScoped('legalizacao_instituicoes',instituicaoId);
  const subtipo=upper(payload.subtipo||'CREDENCIAMENTO');if(!['CREDENCIAMENTO','RECREDENCIAMENTO'].includes(subtipo))throw new Error('Tipo de procedimento inválido.');const regimeCadastro=upper(inst.regime_educacional||'');const regime=regimeCadastro||upper(payload.regime_educacional||'EDUCACAO_BASICA');if(!['EDUCACAO_BASICA','EDUCACAO_PROFISSIONAL_TECNICA'].includes(regime))throw new Error('Regime educacional inválido no cadastro da instituição.');if(regimeCadastro&&payload.regime_educacional&&upper(payload.regime_educacional)!==regimeCadastro)throw new Error('O regime do credenciamento deve seguir a classificação definida no cadastro da instituição.');
...
async function converterCredenciamentoParaTecnico(processoId){
  assertOperacaoNte();assertAccess();const c=client(),p=await processoCredenciamento(processoId);
  if(upper(p.regime_educacional)==='EDUCACAO_PROFISSIONAL_TECNICA')return p;
  if(clean(p.numero_sei))throw new Error('O Processo SEI já foi registrado. A alteração do regime deve ocorrer antes do encaminhamento documental.');
  const {data:ofertas,error:eo}=await c.from('legalizacao_processos_ofertas').select('id').eq('processo_id',p.id).limit(1);if(eo)throw eo;
  if((ofertas||[]).length)throw new Error('Já existe oferta vinculada a este credenciamento. Remova/corrija a oferta antes de alterar o regime.');
  let cq=c.from('legalizacao_checklist_catalogo').select('*').eq('tipo_processo','CREDENCIAMENTO_PROFISSIONAL').eq('ativo',true).order('ordem',{ascending:true});
  const inst=await oneScoped('legalizacao_instituicoes',p.instituicao_id);cq=upper(inst.tipo_cadastro)==='PRIVADA'?cq.eq('aplica_privada',true):cq.eq('aplica_publica',true);
  const {data:catalogo,error:ec}=await cq;if(ec)throw ec;if(!(catalogo||[]).length)throw new Error('O checklist de Credenciamento da Educação Profissional Técnica ainda não está parametrizado no catálogo.');
  const {error:del}=await c.from('legalizacao_checklist_processo').delete().eq('processo_id',p.id);if(del)throw del;
  const rows=catalogo.map(x=>({processo_id:p.id,catalogo_id:x.id,status:'NAO_APRESENTADO'}));const {error:ins}=await c.from('legalizacao_checklist_processo').insert(rows);if(ins)throw ins;
  const {data:upd,error:eu}=await c.from('legalizacao_processos').update({regime_educacional:'EDUCACAO_PROFISSIONAL_TECNICA',atualizado_por_id:currentUserId(),updated_at:new Date().toISOString()}).eq('id',p.id).select('*').single();if(eu)throw eu;
  await historicoProcesso(p.id,'ALTERACAO_REGIME','Procedimento ajustado para Credenciamento de Educação Profissional Técnica de Nível Médio, preservando o processo existente.',{regime_anterior:upper(p.regime_educacional||'EDUCACAO_BASICA'),regime_novo:'EDUCACAO_PROFISSIONAL_TECNICA'});resumoCache=null;return upd;
}
