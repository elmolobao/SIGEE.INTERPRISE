...
async function agendarInspecao(inspecaoId,dataAgendada){assertAccess();const c=client();const {data:i,error:ei}=await c.from('legalizacao_inspecoes').select('*').eq('id',inspecaoId).single();if(ei)throw ei;if(!master()&&Number(i.nte_id)!==Number(nteId()))throw new Error('Inspeção fora da sua abrangência.');if(!dataAgendada)throw new Error('Informe a data da inspeção.');const now=new Date().toISOString();const {data,error}=await c.from('legalizacao_inspecoes').update({data_agendada:dataAgendada,status:'AGENDADA',responsavel_id:i.responsavel_id||currentUserId(),updated_at:now}).eq('id',inspecaoId).select('*').single();if(error)throw error;await historicoProcesso(i.processo_id,'INSPECAO_AGENDADA','Inspeção agendada.',{data_agendada:dataAgendada});
try{const pr=await c.from('legalizacao_processos').select('id,numero_sei,instituicao_id').eq('id',i.processo_id).maybeSingle();let nome='Instituição';if(pr.data?.instituicao_id){const ir=await c.from('legalizacao_instituicoes').select('nome_instituicao,nome').eq('id',pr.data.instituicao_id).maybeSingle();nome=ir.data?.nome_instituicao||ir.data?.nome||nome;}const u=user();const row={tipo:'INSPECAO_REGULATORIA',titulo:'Inspeção regulatória — '+nome,inicio:dataAgendada,fim:dataAgendada,modalidade:'PRESENCIAL',situacao:'AGENDADO',prioridade:'NORMAL',ntes:[Number(i.nte_id)].filter(Boolean),dominio:'LEGALIZACAO',legalizacao_processo_id:i.processo_id,legalizacao_inspecao_id:i.id,numero_sei:pr.data?.numero_sei||null,tecnico_id:String(i.responsavel_id||currentUserId()||'')||null,tecnico_nome:u?.nome||u?.name||u?.email||null,criado_por_id:currentUserId(),criado_por_nome:u?.nome||u?.name||'',criado_por_email:String(u?.email||'').toLowerCase()||null,updated_at:now};await c.from('gt_agenda').upsert(row,{onConflict:'legalizacao_inspecao_id'});}catch(ae){console.warn('[SIGEE Agenda] Inspeção regulatória não espelhada:',ae?.message||ae);}return data;}
...
async function confirmarAtoImportado(importacaoId,escolaId=null,ajustes={}){
  ...
  // O processo SEI é vínculo auxiliar. Falhas nessa etapa não anulam a confirmação do DOE nem a atualização do cadastro.
  if(clean(r.numero_processo)){
    try{
      const {data:procs,error:ep}=await c.from('legalizacao_processos').select('id,tipo,subtipo,status,etapa_atual,numero_sei,instituicao_id').eq('instituicao_id',inst.id).eq('numero_sei',clean(r.numero_processo)).order('created_at',{ascending:false}).limit(10);if(ep)throw ep;
      const candidatos=(procs||[]).filter(x=>!['CONCLUIDO','CANCELADO'].includes(upper(x.status)));
      if(candidatos.length===1){
        const p0=candidatos[0],upd={tipo_ato:clean(r.tipo_ato)||clean(r.ato)||p0.subtipo||p0.tipo,numero_ato:clean(r.numero_publicacao),data_publicacao:clean(r.data_publicacao),referencia_doe:`Importação DOE: ${clean(r.arquivo_origem)}`,vigencia_inicio:clean(r.vigencia_inicio),vigencia_fim:clean(r.vigencia_fim),publicado_em:now,publicado_por_id:currentUserId(),status:'CONCLUIDO',etapa_atual:'ATO_PUBLICADO',atualizado_por_id:currentUserId(),updated_at:now};
        if(upper(p0.tipo)==='CREDENCIAMENTO'){upd.etapa_atual='CREDENCIAMENTO_CONCLUIDO';upd.credenciamento_concluido_em=now;upd.credenciamento_concluido_por_id=currentUserId();}
        const {error:eup}=await c.from('legalizacao_processos').update(upd).eq('id',p0.id);if(eup)throw eup;
        await historicoProcesso(p0.id,'PUBLICACAO_DOE_IMPORTADA','Publicação do Diário Oficial importada, conferida e vinculada ao Processo SEI.',{numero_ato:r.numero_publicacao,data_publicacao:r.data_publicacao,importacao_id:r.id});
