...
async function atualizarInstituicao(instituicaoId,payload){
  ...
  if(!registro.nome_instituicao)throw new Error('Nome da instituição é obrigatório.');if(!['PUBLICA','PRIVADA'].includes(tipo))throw new Error('Tipo da instituição é obrigatório.');
  if(!importado){if(!registro.ano_base_matriculas)throw new Error('Ano-base é obrigatório.');if(!registro.logradouro)throw new Error('Logradouro é obrigatório.');if(registro.cep.length!==8)throw new Error('CEP inválido. Informe 8 dígitos.');if(!registro.municipio)throw new Error('Município é obrigatório.');if(!['PROPRIO','ALUGADO'].includes(registro.situacao_imovel))throw new Error('Informe obrigatoriamente se o imóvel é próprio ou alugado.');if(!cnpjValido(payload.mantenedora_cnpj))throw new Error('CNPJ da mantenedora inválido.');if(!cpfValido(payload.diretor_cpf))throw new Error('CPF do diretor inválido.');if(!cpfValido(payload.secretario_cpf))throw new Error('CPF do secretário inválido.');}
  else {if(registro.cep&&registro.cep.length!==8)throw new Error('CEP inválido. Informe 8 dígitos.');if(registro.situacao_imovel&&!['PROPRIO','ALUGADO'].includes(registro.situacao_imovel))throw new Error('Situação do imóvel inválida.');if(clean(payload.mantenedora_cnpj)&&!cnpjValido(payload.mantenedora_cnpj))throw new Error('CNPJ da mantenedora inválido.');if(clean(payload.diretor_cpf)&&!cpfValido(payload.diretor_cpf))throw new Error('CPF do diretor inválido.');if(clean(payload.secretario_cpf)&&!cpfValido(payload.secretario_cpf))throw new Error('CPF do secretário inválido.');}
  const obrigatorios={'Razão social da mantenedora':payload.mantenedora_razao_social,'CNPJ da mantenedora':payload.mantenedora_cnpj,'Representante legal da mantenedora':payload.mantenedora_representante,'Telefone da mantenedora':payload.mantenedora_telefone,'WhatsApp da mantenedora':payload.mantenedora_whatsapp,'E-mail da mantenedora':payload.mantenedora_email,'Município da mantenedora':payload.mantenedora_municipio,'Nome do diretor':payload.diretor_nome,'CPF do diretor':payload.diretor_cpf,'Telefone do diretor':payload.diretor_telefone,'WhatsApp do diretor':payload.diretor_whatsapp,'E-mail do diretor':payload.diretor_email,'Nome do secretário':payload.secretario_nome,'CPF do secretário':payload.secretario_cpf,'Telefone do secretário':payload.secretario_telefone,'WhatsApp do secretário':payload.secretario_whatsapp,'E-mail do secretário':payload.secretario_email};if(!importado){const faltantes=Object.entries(obrigatorios).filter(([,v])=>!clean(v)).map(([k])=>k);if(faltantes.length)throw new Error('Campos obrigatórios pendentes: '+faltantes.join(', ')+'.');}
  ...
  for(const [rotulo,campo,obrig] of [['Telefone da escola','telefone',false],['WhatsApp da escola','whatsapp',false],['Telefone da mantenedora','mantenedora_telefone',!importado],['WhatsApp da mantenedora','mantenedora_whatsapp',!importado],['Telefone do diretor','diretor_telefone',!importado],['WhatsApp do diretor','diretor_whatsapp',!importado],['Telefone do secretário','secretario_telefone',!importado],['WhatsApp do secretário','secretario_whatsapp',!importado]])if((clean(payload[campo])||obrig)&&!telefoneValido(payload[campo],obrig))throw new Error(rotulo+' inválido. Informe DDD + número.');
  ...
  const temMantenedora=Object.entries(mantenedora).some(([k,v])=>k!=='uf'&&!!clean(v));let rm=await c.from('legalizacao_mantenedoras').select('id').eq('instituicao_id',anterior.id).order('created_at',{ascending:false}).limit(1);if(rm.error)throw rm.error;if(rm.data?.[0]){const r=await c.from('legalizacao_mantenedoras').update(mantenedora).eq('id',rm.data[0].id);if(r.error)throw r.error;}else if(temMantenedora){const r=await c.from('legalizacao_mantenedoras').insert({instituicao_id:anterior.id,...mantenedora});if(r.error)throw r.error;}
  for(const tipoResp of ['DIRETOR','SECRETARIO']){const pfx=tipoResp==='DIRETOR'?'diretor':'secretario',resp={tipo:tipoResp,nome:clean(payload[pfx+'_nome']),cpf:digits(payload[pfx+'_cpf'],11),telefone:digits(payload[pfx+'_telefone'],11),whatsapp:digits(payload[pfx+'_whatsapp'],11),email:clean(payload[pfx+'_email'])},temResp=Object.entries(resp).some(([k,v])=>k!=='tipo'&&!!clean(v));const rr=await c.from('legalizacao_responsaveis').select('id').eq('instituicao_id',anterior.id).eq('tipo',tipoResp).order('created_at',{ascending:false}).limit(1);if(rr.error)throw rr.error;if(rr.data?.[0]){const r=await c.from('legalizacao_responsaveis').update(resp).eq('id',rr.data[0].id);if(r.error)throw r.error;}else if(temResp){const r=await c.from('legalizacao_responsaveis').insert({instituicao_id:anterior.id,...resp});if(r.error)throw r.error;}}
...
async function obterProntuario(instituicaoId){
  ...
  return {instituicao,mantenedoras,responsaveis,carimbos,ofertas,processos,fiscalizacoes,inspecoes,handoffs,atosLegais,credenciamento:cred,checklist,credOfertas,ofertaChecklist,descredenciamento,checklistDescredenciamento};
}
...
async function iniciarCredenciamento(instituicaoId,payload={}){
  ...
  await historicoProcesso(p.id,'ABERTURA',`${subtipo==='RECREDENCIAMENTO'?'Recredenciamento':'Credenciamento'} iniciado no SIGEE para recebimento documental.`,{numero_sei:null,tipo_cadastro:inst.tipo_cadastro,subtipo,prazo_etapa:clean(payload.prazo_etapa)});resumoCache=null;return p;
}
...
window.SIGEE_LEGALIZACAO_SERVICE=Object.freeze({listarInstituicoes,consultarInstituicoes,contarInstituicoes,resumo,listarBasePendenciasRegulatorias,listarNtes,listarInspecoesGerais,listarHistoricoRegulatorio,listarProcessosRegulatorios,listarDescredenciamentosRegulatorios,garantirChecklistDescredenciamento,listarOfertasRegulatorias,listarProcessosOfertaRegulatorios,listarCatalogoOfertas,iniciarProcedimentoOferta,adicionarOfertasCredenciamento,atualizarRequisitoOferta,listarCarimbosRegulatorios,listarAtosHistoricosControle,criarInstituicao,atualizarInstituicao,confirmarCadastroMigrado,habilitarProntuario,obterProntuario,obterProntuarioPorEscola,iniciarCredenciamento,iniciarDescredenciamento,registrarProcessoSeiCredenciamento,atualizarChecklist,emitirDiligencia,retomarAnalise,prepararInspecao,agendarInspecao,atualizarItemInspecao,registrarRealizacaoInspecao,concluirInspecao,concluirAnaliseFinal,registrarPublicacao,concluirCredenciamento,importarAtosLote,listarAtosImportados,resumoImportacaoAtos,confirmarAtoImportado,integrarAtosIdentificados,listarAtosInstituicao,ehMaster:master,nteId});
})(window);
