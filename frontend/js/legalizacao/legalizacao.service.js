/** SIGEE Enterprise RC12.0.4 — Análise final, publicação e conclusão do credenciamento. */
(function(window){
'use strict';
if(window.__SIGEE_LEGALIZACAO_SERVICE_RC1204__)return;
window.__SIGEE_LEGALIZACAO_SERVICE_RC1204__=true;
...
function escolaParaInstituicao(escola,extensao=null){const dep=escola?.dependencia_adm||escola?.dependencia||'',priv=depPrivada(dep);return {...(extensao||{}),id:extensao?.id||null,prontuario_id:extensao?.id||null,escola_id:escola?.id,nte_id:escola?.nte_id??extensao?.nte_id,nome_instituicao:escola?.nome_escola||escola?.nome||extensao?.nome_instituicao,cod_inep:escola?.cod_mec||extensao?.cod_inep,municipio:escola?.municipio||extensao?.municipio,tipo_cadastro:priv?'PRIVADA':'PUBLICA',rede:priv?'PRIVADA':(upper(dep).includes('MUNICIPAL')?'MUNICIPAL':(upper(dep).includes('FEDERAL')?'FEDERAL':'ESTADUAL')),natureza:priv?'PRIVADA':`PUBLICA_${upper(dep)||'NAO_CLASSIFICADA'}`,dependencia_adm:dep,situacao_funcional:escola?.situacao_funcional||escola?.situacao||null,situacao_regulatoria:extensao?.situacao_regulatoria||'NAO_HABILITADO',prontuario_habilitado:Boolean(extensao?.id),origem:extensao?.origem||'CATALOGO_SIGEE'};}
...
window.SIGEE_LEGALIZACAO_SERVICE=Object.freeze({listarInstituicoes,consultarInstituicoes,contarInstituicoes,resumo,listarNtes,listarInspecoesGerais,listarHistoricoRegulatorio,listarProcessosRegulatorios,listarOfertasRegulatorias,listarCarimbosRegulatorios,criarInstituicao,confirmarCadastroMigrado,habilitarProntuario,obterProntuario,iniciarCredenciamento,atualizarChecklist,emitirDiligencia,retomarAnalise,prepararInspecao,agendarInspecao,atualizarItemInspecao,registrarRealizacaoInspecao,concluirInspecao,concluirAnaliseFinal,registrarPublicacao,concluirCredenciamento,ehMaster:master,nteId});
})(window);
