...
async function vincularOfertasAoCredenciamento(processo,inst,payload={}){
  ...
  const ehAutorizacaoInicial=r=>{const t=upper([r.codigo,r.grupo,r.titulo,r.descricao,r.tipo_processo,r.subtipo_aplicavel].filter(Boolean).join(' '));return !/(RENOVACAO|RENOVAÇÃO|RECONHECIMENTO)/.test(t);};const requisitosAplicaveis=(reqCat||[]).filter(ehAutorizacaoInicial);
