...
async function listarAtosImportados(status=''){assertAccess();if(!podeGerirDoe())throw new Error('A conferência de importações é autorizada apenas para os perfis Master e SEC.');const c=client();let q=c.from('legalizacao_atos_importacao').select('id,lote_id,arquivo_origem,linha_origem,nte_numero,municipio,escola_nome,ato,tipo_ato,numero_publicacao,data_publicacao,numero_processo,vigencia_inicio,vigencia_fim,vigencia_origem,status_match,escola_id,instituicao_id,cnpj_extraido,detalhe,endereco_extraido,created_at,confirmado_em,confirmado_por_id').order('id',{ascending:false}).limit(1000);const st=upper(status);if(st==='ATIVAS')q=q.in('status_match',['IDENTIFICADO','PENDENTE_CONFERENCIA','AMBIGUO']);else if(st)q=q.eq('status_match',st);const {data,error}=await q;if(error)throw error;return data||[];}
