...
function modal(ev={}){
 ...
 m.querySelector('#gta-nte-todos').addEventListener('click',()=>{checks().filter(x=>!x.closest('.gt-nte-option').hidden).forEach(x=>x.checked=true);atualizarNtes();});
...
window.SIGEE_TERRITORIAL_AGENDA=Object.freeze({render,carregar,modal,versao:'GT-02.1'});
