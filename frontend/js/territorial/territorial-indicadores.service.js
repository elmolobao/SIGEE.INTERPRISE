...
function mediaSatisfacao(rows){const r=rows.filter(x=>x.score_satisfacao!=null);return{nota:r.length?Math.round(r.reduce((s,x)=>s+Number(x.score_satisfacao),0)/r.length*10)/10:null,amostra:r.length};}
...
async function carregar(){const c=exigir();const [ma,ag,pe,ac]=await Promise.all([
 ...
 return ntes().map(n=>{const num=n.numero,m=monitor.filter(x=>Number(x.nte_numero)===num),p=pesquisa.filter(x=>Number(x.nte_numero)===num),ev=eventosNte(agenda,num);const tec=notaTecnica(m),sat=mediaSatisfacao(p),status=statusFormacao(agenda,num),pos=status==='CONCLUIDA';const indice=consolidado(tec.nota,sat.nota),reunioes=ev.filter(x=>x.tipo==='REUNIAO'&&x.situacao==='REALIZADO').length,visitas=ev.filter(x=>x.tipo==='VISITA_TECNICA'&&x.situacao==='REALIZADO').length,negativas=m.filter(x=>x.avaliacao==='NEGATIVA').length,positivas=m.filter(x=>x.avaliacao==='POSITIVA').length,prior=pos?{nota:null,classe:'NAO_APLICA'}:prioridadeFormacao(tec.nota,sat.nota,acoesNte[num]||0,maxAcoes);return{...n,statusFormacao:status,fase:pos?'POS_FORMACAO':'PRE_FORMACAO',notaTecnica:tec.nota,amostraTecnica:tec.amostra,satisfacao:sat.nota,amostraSatisfacao:sat.amostra,indiceConsolidado:indice,classeDesempenho:pos?classeDesempenho(indice):'NAO_APLICA',prioridadeNota:prior.nota,prioridadeClasse:prior.classe,positivas,negativas,reunioes,visitas,acoes:acoesNte[num]||0,producao:prodMap.get(num)||0};});
}
window.SIGEE_TERRITORIAL_INDICADORES_SERVICE=Object.freeze({carregar,master,PESOS,classeDesempenho,versao:'GT-06.0'});
})(window);
