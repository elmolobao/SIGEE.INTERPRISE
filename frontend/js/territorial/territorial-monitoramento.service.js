/** SIGEE Enterprise — GT-03.1 Monitoramento Territorial orientado. */
(function(window){
'use strict';
if(window.SIGEE_TERRITORIAL_MONITORAMENTO_SERVICE?.versao==='GT-03.1') return;
...
async function salvarOcorrencia(payload){
  ...
  const fase=await faseDoNte(nte);const registro={nte_numero:nte,fase,natureza:'OCORRENCIA',data_registro:payload.data_registro||new Date().toISOString(),titulo:txt(payload.titulo),descricao:txt(payload.descricao),categoria:String(payload.categoria||'PROCEDIMENTO').toUpperCase(),relevancia:String(payload.relevancia||'INFORMATIVA').toUpperCase(),processo_id:payload.processo_id?Number(payload.processo_id):null,codigo_sigee:txt(payload.codigo_sigee)||null,aluno_nome:txt(payload.aluno_nome)||null,conteudo_formacao:fase==='POS_FORMACAO'?String(payload.conteudo_formacao||'NAO').toUpperCase():'NAO_SE_APLICA',resultado:String(payload.resultado||'EM_ACOMPANHAMENTO').toUpperCase(),prazo:payload.prazo||null,concluido_at:payload.concluido_at||null,evidencia_referencia:txt(payload.evidencia_referencia)||null,observacoes:txt(payload.observacoes)||null,updated_at:new Date().toISOString()};
