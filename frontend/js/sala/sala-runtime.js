...
function prepararProntuario(p){if(!p)return;window.SIGEE_ESCOPO?.exigirRegistro?.(p,usuario());window.processosDB=Array.isArray(window.processosDB)?window.processosDB:[];const i=window.processosDB.findIndex(x=>String(x.id)===String(p.id));if(i>=0)window.processosDB[i]={...window.processosDB[i],...p};else window.processosDB.push(p);window.abrirProntuarioSIGEE?.(p.id);}
