(function(global, document){
  'use strict';
  if(global.SIGEE_CIO_WIDGET?.version==='RC6.3.0')return;
  ...
  function render(resultado){
    ...
    const fila=(m.filaPrioritaria||[]).slice(0,8).map((x,i)=>`<button data-cio-prontuario-rapido="${esc(x.id)}"><span>${i+1}</span><div><b>${esc(x.codigo||x.id||'Processo')}</b><small>${esc(nteValor(x))} · ${esc(x.etapa)} · ${esc(responsavelExibicao(x))}</small></div><strong class="risco-${esc((x.nivel||'normal').toLowerCase())}">${esc(x.nivel)}</strong></button>`).join('');
    const territorial=(m.territorial||[]).slice(0,10).map(x=>`<button data-cio-lista="NTE:${esc(x.nte)}"><span class="sigee-cio-nte-badge">${esc(x.nte)}</span><b>${x.criticos} críticos</b><small>${x.total} ativos · ${x.semResponsavel} sem responsável</small><i>${x.indiceRisco}</i></button>`).join('');
    return `<div class="sigee-cio-widget-brief"><strong>${esc(resultado.resumo?.saudacao||'Resumo operacional')}</strong><p>${esc(resultado.resumo?.texto||'Análise concluída.')}</p></div>
      ...
      <section class="sigee-cio-section sigee-cio-territorial-section"><div class="sigee-cio-section-title"><h3>Monitoramento territorial</h3><button type="button" data-cio-lista="SEM_RESP">${m.semResponsavel?.length||0} aguardando atribuição</button></div><div class="sigee-cio-territorial">${territorial||'<p>Sem dados territoriais.</p>'}</div></section>
  ...
  global.SIGEE_CIO_WIDGET=Object.freeze({version:'RC6.3.0',instalar,carregar,abrir,abrirProntuario,responsavelExibicao});
