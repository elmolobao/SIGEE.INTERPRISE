(function (global) {
  ...
  if (global.__SIGEE_CIO_BOOTSTRAP__?.version === 'RC6.3.1') return;
  ...
  function carregarScript(caminho) {
    return new Promise((resolve, reject) => {
      ...
      script.id = id;
      script.async = false;
      script.src = 'js/' + caminho + '?v=RC6.3.1';
      script.onload = () => {
  ...
  async function carregarModulos() {
    ...
    state.loadingPromise = (async () => {
      ...
      state.state = 'ready';
      console.info('[SIGEE RC6.3.1] Motor de Gestão Operacional carregado sob demanda.');
    })().catch((erro) => {
      state.state = 'error';
  ...
  console.info('[SIGEE RC6.3.1] Infraestrutura leve do CIO ativa.');
})(window);
